...
def wishMe():
    ...
    if(hour>=0) and (hour<=12) and ('AM' in t):
        speak(f"Good morning Srinivas, it's {day} and the time is {t}")
    elif(hour>=12)  and (hour<=16) and ('PM' in t):
        speak(f"Good afternoon Srinivas, it's {day} and the time is {t}")
    else:
        speak(f"Good evening Srinivas, it's {day} and the time is {t}")
...
def condition():
    ...
    if percentage>=80:
        speak("Boss we have enough energy to stop charging")
